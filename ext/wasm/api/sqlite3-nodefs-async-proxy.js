...
const wMsg = (type)=>parentPort.postMessage({type});
const wLog = (...args)=>parentPort.postMessage({type:'log',msg: (args)});
...
const error =  (...args)=>console.log(0, ...args);
...
const vfsAsyncImpls = {
  ...
  xFileSize: async function(fid/*sqlite3_file pointer*/){
    ...
    try{
      ...
      // rc = (await fh.fileHandle.stat()).size;
      rc = fs.fstatSync(fh.fileHandle).size;
  ...
  xOpen: async function(fid/*sqlite3_file pointer*/, filename,
    ...
    try{
      ...
      const fileHandle = fs.openSync(resolvedFilename, openFlags);
      /**
      ...
      */
      ...
      __openFiles[fid] = Object.assign(Object.create(null),{
        dirHandle,
        fileHandle,
        filenameAbs: resolvedFilename,
        filenamePart: path.basename(resolvedFilename),
  ...
  xTruncate: async function(fid/*sqlite3_file pointer*/,size){
    ...
    try{
      ...
      fs.ftruncateSync(fh.fileHandle, size);
    }catch(e){
